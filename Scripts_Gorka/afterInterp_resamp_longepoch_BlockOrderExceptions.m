...
% ================================================================================
%%% Input are files 009, 019,107 or 117 after bp filter
    %if they have interpolated channels 
% Resample to 256Hz
% Rename events (to that of subj with even number), epoch (-500 1550) and remove baseline
...
for G = G; 
    if          G == 0 
        DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Pretest\Visual_word_analysis\Pretest_dyslexia\InterpolatedMin';
        elseif G == 1
        DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Pretest\Visual_word_analysis\Pretest_school\InterpolatedMin';
        elseif G == 2
        DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Control\Visual_word_analysis\Control_school\InterpolatedMin';
        elseif G == 3
        DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Control\Visual_word_analysis\Control_dyslexia\InterpolatedMin';  
        elseif G == 4
        DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Posttest\Visual_word_analysis\Posttest_dyslexia\InterpolatedMin';
        elseif G == 5
        [DIRNAME] = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab\Dyslexie_Posttest\Visual_word_analysis\Posttest_school\InterpolatedMin';
    end
...
for N = [N]; 
  if N < 10
    [FILENAME] =['s',num2str(G),'0',num2str(N),'-vwr-ch_rej-bp-interp'];
    elseif N >=10 && N<100   
    [FILENAME] =['s',num2str(G),num2str(N),'-vwr-ch_rej-bp-interp'];
  end
...
%%%Save epoched files
FILENAME3 = strrep (FILENAME,'ch_rej-bp-interp','interp-Epoched');
