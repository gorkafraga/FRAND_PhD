% SPLIT AND PLOT AVERAGES IN INTERPOLATED DATA
% ============================================
% Split: file in events (21, 22,23,24)
% Plot: averages of 21 & 22 versus average of 23 & 24
% Input/output directory: "Interpolation"folder
% Input files: sXXX-vwr-ICA-pruned-br-interp-lp-refAvg.set
% Output files:  sXXX-vwr-ICA-pruned-br-interp-lp-refAvg-e21.set
                % sXXX-vwr-ICA-pruned-br-interp-lp-refAvg-e22.set
                % (...) 
                % PLOT : sXXX-wordsVsSymbols-interp.fig
%--------------------------------------------------------------------------
...
for N = N; 
    ...
    [FILENAME] =['s',num2str(G),'0',num2str(N),'-vwr-ICA-pruned-br-interp-lp-refAvg'];
         [FILES] =['s',num2str(G),'0',num2str(N),'-vwr-ICA-pruned-br-interp-lp-refAvg-'];
    elseif N >=10 && N<100   
    [FILENAME] =['s',num2str(G),num2str(N),'-vwr-ICA-pruned-br-interp-lp-refAvg'];
        [FILES] =['s',num2str(G),num2str(N),'-vwr-ICA-pruned-br-interp-lp-refAvg-'];
    end
...
FIGURENAME = [FIGURENAME, '-wordsVsSymbols-interp'];
