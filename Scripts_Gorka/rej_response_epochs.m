...
% ================================================================================
% Automatically rej trials including the event 12 (response trials)
% Manually define group code (G) and subj number (N)
% Input files are "epoched"files. 
% Output files ending is "rejResp". 
% There is a counter  desactivated at the end (use "counter_all"scripts)
%--------------------------------------------------------------------------
...
EEG = pop_saveset (EEG, FILENAME2,DIRNAME);
 % Counter commented; the count is done with another a script: count_all...
%     codes=zeros(5,1);
%     counts=codes;
%     counter=1;
% 
