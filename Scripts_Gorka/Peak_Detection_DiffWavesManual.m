...
close all
eeglab
DIROUTPUT = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab11\PeakDetectionDiffWaves';
%%%%--------------------------------------------------------------------------
  range = 25;
  int=1;
%--------------------------------------------------------------------------
...
for G = G; 
    ...
    N1Latencies = cell(1,1+length(COI)*2);
    N1Amplitudes = cell(1,1+length(COI)*2);
    P1Latencies = cell(1,1+length(COI)*2);
    P1Amplitudes = cell(1,1+length(COI)*2);
...
%%% (SW,LW,SS,LS) and value of peak(latency or amplitude)
    ...
    for l = 1:length(COI);
        C = labels(COI(l));
         N1Latencies(1,1+l) = {[cell2mat(C),'-ShortDif-L']};
         N1Amplitudes(1,1+l) = {[cell2mat(C),'-ShortDif-A']};
         P1Latencies(1,1+l) = {[cell2mat(C),'-ShortDif-L']};
         P1Amplitudes(1,1+l) = {[cell2mat(C),'-ShortDif-A']};
    end
    for l = 1:length(COI)
        C = labels(COI(l));
       N1Latencies (1,(1+length(COI))+l) = {[cell2mat(C),'-LongDif-L']};
       N1Amplitudes(1,(1+length(COI))+l) = {[cell2mat(C),'-LongDif-A']};
       P1Latencies (1,(1+length(COI))+l) = {[cell2mat(C),'-LongDif-L']};
       P1Amplitudes(1,(1+length(COI))+l) = {[cell2mat(C),'-LongDif-A']};
    end
...
%      [filesSW] =['s',num2str(G),num2str(n),'*e21.set'];
%      [filesLW] =['s',num2str(G),num2str(n),'*e22.set'];
%      [filesSS] =['s',num2str(G),num2str(n),'*e23.set'];
%      [filesLS] =['s',num2str(G),num2str(n),'*e24.set'];
  end
% FILES = [FILES '*e2*.set'];
...
%defineCondition
     if ~isempty(strfind(FILENAME1,'e21'))
         FILENAME2 = strrep (FILENAME1, 'e21', 'e23'); 
          condition = 'ShortWords';
        elseif ~isempty(strfind(FILENAME1,'e22'))
             FILENAME2 = strrep (FILENAME1, 'e22', 'e24');
            condition = 'LongWords';
...
%%
 cd (DIRNAME)
   ...
   if  ~isempty(strfind(FILENAME1,'e21')) || ~isempty(strfind(FILENAME1,'e22'))
   %% compute averages of all trials per electrode and data point
 
   % load file 1 in EEGlab 
...
% Go through eleectordes
  ...
  %-------------------------------------------------------------    
   % load file 2 in EEGlab
     EEG = pop_loadset('filename',FILENAME2);

eeglab redraw
    % Take the information on electrodes and timepoints from current EEG
        % dataset
        Electrodes2=EEG.data(1:64,:,:);
        time2=EEG.times;
        X2=zeros(64,EEG.trials);% X=zeros(1:64,EEG.trials)gives "warning input must be scalars"
       AvgEl2=zeros(64,EEG.pnts,1);%AvgEl=zeros(1:64,EEG.pnts,1)gives "warning input must be scalars"
...
% Go through eleectordes
        for k =1:64
            ...
            for i=1:(EEG.pnts)
                ...
                for j=1:(EEG.trials)
                   % For an electrode k, make a vector X with the point i for all
                   % the epochs
                   X2(k,j)=Electrodes2(k,i,j);     
                end
                                
                % Get an average value for the point i from all the epochs
                Y2(k,1)=sum(X2(k,:))/EEG.trials;
  ...
  for k = COI(1:end); % loop through ch of interest
        ...
        for i = 1:length(time); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% P1
            if time(1,i) >   100.000 && time(1,i)< 105.000; 
   ...
   %==== calculate peak to be checked 
    %First peak P1
      ...
      %Second peak N1
        ...
        plot(X, AvgEl(k,:),'b','LineStyle', ':');hold on;
        plot(X, AvgEl2(k,:),'r','LineStyle', ':');hold on;
        ...
        %set the Title
        PLOTNAME = [FILENAME1(1:4),TABLENAME,'-',strrep(condition,'Words','Diff'),cell2mat(labels(k))];
         % set ticks and axes 
        ...
        line([0 ; 0],[-50 50], 'linewidth',1,'LineStyle', '-','color', 'k');hold on; 
        line([-500 ; 1500],[0 0], 'linewidth',1,'LineStyle', '-','color', 'k');hold on; 
        scatter (peak_latency, peak_amplitude,'filled','marker', '+','markeredgecolor','r','markerfacecolor','r','linewidth',2);hold on;
        scatter (peak_latency2, -peak_amplitude2,'filled','marker', 'x','markeredgecolor','r','markerfacecolor','r','linewidth',2);hold on;
...
%% now save peaks Amplitudes & Latencies in the corresponding columns
 ...
 % save to file with name   
    suffixAmp = ['-',num2str(length(COI)),'ch','-P1DifAmp'];
    suffixLat = ['-',num2str(length(COI)),'ch','-P1DifLat'];
    suffix =  ['-',num2str(length(COI)),'ch','-P1Dif'];
    suffixAmp2 = ['-',num2str(length(COI)),'ch','-N1DifAmp'];
    suffixLat2 = ['-',num2str(length(COI)),'ch','-N1DifLat'];
    suffix2 =  ['-',num2str(length(COI)),'ch','-N1Dif'];
