...
% Renaming the events of the VWR-task for ODD subjects.
...
% For odd subjects we have the following sequence:
% TF FT FF TT FF TT TF FT
...
for subjectsi = 1 : length(subjects)
    ...
    for i=1:length(EEG.urevent)
        ...
        if counter <=40;                        % the first 40 stimulus events are coded 21 for "ShortWords" [TF]
            EEG.event(i).type = 21;
        end

        if counter > 40 && counter <=80;        % the folowwing 40 stimulus events are coded 24 for "LongSymbols" [FT]
            EEG.event(i).type = 24;
        end

        if counter > 80 && counter <=120;       % the following 40 stimulus events are coded 22 "LongWords" [FF]
            EEG.event(i).type = 22;
        end
        ...
        if counter > 160 && counter <=200;      % the following 40 stimulus events are coded 22 for "LongWords" [FF]
            EEG.event(i).type = 22;
        end

        if counter > 200 && counter <=240;      % the following 40 stimulus events are coded 23 for "ShortSymbols" [TT]
            EEG.event(i).type = 23;
        end

        if counter > 240 && counter <=280;      % the following 40 stimulus events are coded 21 for "ShortWords" [TF]
            EEG.event(i).type = 21;
        end

        if counter > 280 && counter <=320;      % the folowwing 40 stimulus events are coded 24 for "LongSymbols" [FT]
            EEG.event(i).type = 24;
        end
