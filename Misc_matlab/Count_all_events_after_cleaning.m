...
% COUNT THE No of EVENTS IN EPOCHED DATA AFTER MANUAL INSPECTION REJECTION OF BAD EPOCHS 
...
FILES = [dir('*EpClean.set')]; %Search in current Dir files 'epclean'
for  J = 1:length(FILES); %J contains the list of the files containing 'epclean'.
  ...
  % Counter of epochs for each event:    
    ...
    if ~isempty(dir(FILENAME1));
        ...
        DIRNAME2 = [DIRNAME, '\Trial_count'];
        FILENAME3 = strrep (FILENAME1, 'EpClean.set', 'trial_count.txt');
            Y = X'; %transpose X 
   ...
   %%%%%% =======NOW SAVE ALL IN ONE TXT FILE nice and tidy
          ...
          % Empty  array to fill in subjects and trials per event 
            CleanTrials = zeros (23,5);
            Events = [21,22,23,24];
            % Events = num2cell(Events);
            CleanTrials(1,2:5) = Events; %% event names as headers
           ...
           % Loop
            ...
            for  k = 1:length(ListTexts); 
                ...
                % subject number in first column
                 sNo = textfile(:,2:4);
                CleanTrials(1+k,1)= str2num(sNo); 
                % Write the number of trials in the next rows
                sCount = load (textfile);
                sCount = sCount(2,2:5);
                CleanTrials(1+k,2:5) = sCount;
                %save to txt file in same folder 
                
                dlmwrite('CleanTrials-all.txt',CleanTrials)
            end
