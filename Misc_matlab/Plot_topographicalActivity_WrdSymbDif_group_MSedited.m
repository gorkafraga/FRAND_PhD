...
cd 'Z:\fraga\EEG_Gorka\Analysis_EEGlab11\PD_noR_new';
...
%Define conditions to loop
  ...
  set(gcf,'Color',[1 1 1] )  
  groupEEG_WrdSymbDif = groupEEG_words-groupEEG_symbs;
 
  % Topographic plot of activity
     times2plot = [0,127,223,341,442];   
     clim = [-5 5];
    ...
    for timei=1:length(times2plot)
        ...
        subplot(4,length(times2plot),timei+length(times2plot)-length(times2plot));
        set(gcf,'Color',[1 1 1] )  % white background
        if timei == 1; 
             text (-2,0,'words-symbols' ,'FontSize',12, 'FontWeight','bold','FontName', 'calibri') ;
        end
...
%                 rgb('IndianRed');rgb('FireBrick');rgb('DarkRed')];    
  ...
  %Plot topography
        topoplot(squeeze(mean(groupEEG_WrdSymbDif(:,timeidx,:),3)),chanlocs,...
    ...
    text(cbpos(1)*1.85, -cbpos(2)*42,'\muV','FontSize', 12, 'FontWeight','bold', 'FontName', 'calibri');
   
    
% OPTIONAL: UNCOMMENT FOR HORIZONTAL COLORBAR (and comment previous cb)
% CB = colorbar('horiz','Position',[0.7500 0.1100 0.05 0.01],'XTick', [clim(1),0,clim(2)],'FontName','Calibri', 'FontSize', 12);
%             cbxlab = get(CB,'XtickLabel'); cbxlab(3,:)=  ['+',num2str(clim(2))];% add + to ylabels in colorbar
%               set (CB,'XTickLabel',cbxlab); 
%     cbpos = get(CB,'Position'); % Get position and add text manually searching for position.
%     text(-cbpos(1)*0.4, -cbpos(2)*56,'\muV','FontSize', 12,'FontWeight','bold', 'FontName', 'calibri');
...
% Save to file in Output Dir   
 ...
 % saveas (gcf,figurename, 'tiff')
  export_fig (sprintf([(figGroupName),'_WordSymbDif_topo_ext']), '-tiff', '-cmyk', '-r300')
  % close gcf
 cd (DIRINPUT)   
