clear all
close all
DIROUTPUT = 'H:\GORKA\Statistical Analysis\FRAND_SPSS\Analysis Responses Excluded\ICAcount';
...
for G = G; %Select group folder in which it will make the count
    DIRNAME = 'Z:\fraga\EEG_Gorka\Analysis_EEGlab11\BackUp_epClean_ICApruned';
%     if  G == 0 
  ...
  EEG = pop_loadset(FILENAME1, DIRNAME);
   % Counter of components:    
    ...
    [C,N] = size (EEG.icaweights); 
     ...
     FILENAME2 = strrep (FILENAME1, 'pruned.set', 'comp_count.txt');        
     dlmwrite(fullfile(DIROUTPUT,FILENAME2), C); 
     
     %==========================================================================
%SAVE CSV FILE WITH ALL SUBJECTS AS ROWS and ICA number in second column

cd  (DIROUTPUT);% go to folder "ICA_count"
